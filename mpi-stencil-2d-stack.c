...
int main(int argc, char** argv) {
	...
	if (argc != 4) {
		printf("mpirun -np <num processes> %s <num iterations> <input file> <stack file> \n",argv[0]);
		return EXIT_FAILURE;
	}
	...
	if (0 < rank && rank < size - 1) {
		...
		write_row_striped_matrix_halo_stack(&fp, (void**)matrixB, MPI_DOUBLE, rows, cols, MPI_COMM_WORLD);
	...
	write_row_striped_matrix_halo_stack(&fp, (void**)matrixA, MPI_DOUBLE, rows, cols, MPI_COMM_WORLD);
