...
int BLOCK_SIZE_HALO(int id, int p, int m)
{
   if (p == 1)
   {
      return BLOCK_SIZE(id, p, m);
   }
    ...
    MPI_Comm comm)      /* IN - Communicator */
{
   ...
   local_rows = BLOCK_SIZE_HALO(id, p, *m);
   ...
   *subs = (void **)my_malloc(id, local_rows * PTR_SIZE);
   ...
   //set entire matrix to -NaN so that halo rows can be seen when printed
   memset(*storage, -1, local_rows * *n * datum_size);
   ...
   //use another pointer to adjust for halo rows
   void *temp_ptr = *storage;
   ...
   if (id == (p - 1))
   {
      ...
      //set entire matrix to -NaN for last process to make sure other processes data isn't leftover in matrix
      memset(*storage, -1, local_rows * *n * datum_size);

      //if more than one process, adjust pointer to account for halo rows, otherwise just read entire matrix into memory
      if (p > 1)
      {
         temp_ptr = temp_ptr + (*n * datum_size);
      }
      fread(temp_ptr, datum_size, BLOCK_SIZE(id, p, *m) * *n, infileptr);
   ...
   else
   {
      ...
      if (id == 0)
      {
         MPI_Recv(*storage, BLOCK_SIZE(id, p, *m) * *n, dtype, p - 1, DATA_MSG, comm, &status);
      }
      else
      {
         //adjust pointer to account for halo rows
         temp_ptr = temp_ptr + (*n * datum_size);
         MPI_Recv(temp_ptr, BLOCK_SIZE(id, p, *m) * *n, dtype, p - 1, DATA_MSG, comm, &status);
      }
   }
}
    ...
    MPI_Comm comm)      /* IN - Communicator */
{
   ...
   if (!id)
   {
      printf("printing submatrix from process: 0\n");
      print_submatrix(a, dtype, local_rows, n);
      if (p > 1)
      {
         datum_size = get_size(dtype);
         max_block_size = BLOCK_SIZE_HALO(p - 1, p, m) + 1;
         bstorage = my_malloc(id,
                              max_block_size * n * datum_size);
         b = (void **)my_malloc(id,
                                max_block_size * datum_size);
         ...
         for (i = 1; i < p; i++)
         {
            MPI_Send(&prompt, 1, MPI_INT, i, PROMPT_MSG,
                     MPI_COMM_WORLD);
            MPI_Recv(bstorage, BLOCK_SIZE_HALO(i, p, m) * n, dtype,
                     i, RESPONSE_MSG, MPI_COMM_WORLD, &status);
            printf("printing submatrix from process: %d\n", i);
            print_submatrix(b, dtype, BLOCK_SIZE_HALO(i, p, m), n);
         }
         free(b);
         free(bstorage);
      }
    ...
    MPI_Comm comm)      /* IN - Communicator */
{
   ...
   if (!id)
   {
      ...
      {
         fwrite(a[0], size, n, fp);
      }
      //fwrite(a,size,n * local_rows,fp);
      //print_submatrix (a, dtype, local_rows, n);
      if (p > 1)
      {
         datum_size = get_size(dtype);
         ...
         for (i = 1; i < p; i++)
         {
            ...
            MPI_Recv(bstorage, BLOCK_SIZE(i, p, m) * n, dtype,
                     i, RESPONSE_MSG, MPI_COMM_WORLD, &status);
            for (int j = 0; j < BLOCK_SIZE(i, p, m); j++)
            {
               fwrite(b[0], size, n, fp);
            }
            //fwrite(b,size,n * BLOCK_SIZE(i,p,m),fp);
            //print_submatrix (b, dtype, BLOCK_SIZE(i,p,m), n);
         }
         free(b);
         free(bstorage);
   ...
   if (!id)
   {
      ...
      if (p > 1)
      {
         ...
         for (i = 1; i < p; i++)
         {
            ...
            if (i < p - 1)
            {
               for (int j = 1; j < BLOCK_SIZE_HALO(i, p, m) - 1; j++)
               {
                  fwrite(b[j], size, n, fp);
               }
            }
            else
            {
               for (int j = 1; j < BLOCK_SIZE_HALO(i, p, m); j++)
               {
                  fwrite(b[j], size, n, fp);
               }
            }
    ...
    MPI_Comm comm)
{
   ...
   datum_size = get_size(dtype);
   local_rows_halo = BLOCK_SIZE_HALO(rank, size, rows);
   ...
   { 
      ...
      MPI_Irecv(*a , cols, dtype, rank - 1, 1, MPI_COMM_WORLD, &reqs[0]);
      MPI_Irecv(*a + ((local_rows_halo - 1) * cols * datum_size), cols, dtype, rank + 1, 1, MPI_COMM_WORLD, &reqs[1]);

      MPI_Isend(*a + (cols * datum_size), cols, dtype, rank - 1, 1, MPI_COMM_WORLD, &reqs[2]);
      MPI_Isend(*a + ((local_rows_halo - 2) * cols * datum_size), cols, dtype, rank + 1, 1, MPI_COMM_WORLD, &reqs[3]);
   }

   if (rank == 0)
   {
      MPI_Irecv(*a + ((local_rows_halo - 1) * cols * datum_size), cols, dtype, rank + 1, 1, MPI_COMM_WORLD, &reqs[0]);

      MPI_Isend(*a + ((local_rows_halo - 2) * cols * datum_size), cols, dtype, rank + 1, 1, MPI_COMM_WORLD, &reqs[1]);
   }
   ...
   if (rank == size - 1)
   {
      ...
      MPI_Isend(*a + (cols * datum_size), cols, dtype, rank - 1, 1, MPI_COMM_WORLD, &reqs[1]);
   }
